...
async def call_taxi(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("📍 Откуда забрать? (геопозиция или текст)", reply_markup=kb)
    await state.set_state(OrderStates.waiting_pickup)
...
async def price(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect('orders.db') as db:
        cursor = await db.execute(
            ...
            (message.from_user.id, phone, data['pickup'], data['destination'], price)
        )
